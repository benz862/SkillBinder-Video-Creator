...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/balance') {
        const r = await fetch(`${HIGGSFIELD_BASE}/users/me`, {
          headers: { 'Authorization': authHeader, 'User-Agent': 'higgsfield-server-js/2.0', 'Content-Type': 'application/json' }
        });
        const text = await r.text();
        let data; try { data = JSON.parse(text); } catch { data = { raw: text }; }
        return new Response(JSON.stringify(data), { status: r.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      if (path === '/api/generate' && request.method === 'POST') {
        const { model, prompt, aspect_ratio, duration, medias } = await request.json();
        ...
        const input = {
          model: model === 'kling3_0' ? 'dop-turbo' : 'dop-standard',
          prompt, aspect_ratio, duration,
          input_images: startImage ? [{ type: 'image_url', image_url: startImage }] : [],
      ...
      if (path === '/api/upload-audio' && request.method === 'POST') {
        ...
        const r = await fetch(`${HIGGSFIELD_BASE}/v1/upload`, {
          method: 'POST',
          headers: { 'Authorization': authHeader, 'User-Agent': 'higgsfield-server-js/2.0', 'Content-Type': contentType, 'X-Filename': filename },
          body: fileBuffer
        });
        const text = await r.text();
        let data; try { data = JSON.parse(text); } catch { data = { raw: text }; }
...
:root{--bg:#0e1218;--surface:#151b24;--surface2:#1c2533;--border:rgba(255,255,255,0.07);--border2:rgba(255,255,255,0.14);--text:#eef0f4;--muted:#6b7585;--navy:#1d3a5c;--navy-light:#2a5080;--gold:#c9a84c;--gold-light:#e8c86e;--gold-dim:rgba(201,168,76,0.12);--red:#e05555;--green:#4caf7d;--radius:10px;--radius-lg:16px}
...
.brand-name{font-family:'Lora',serif;font-size:20px;font-weight:600;color:var(--text)}
.brand-sub{font-family:'DM Mono',monospace;font-size:10px;color:var(--gold);letter-spacing:.1em;text-transform:uppercase}
...
.card-title{font-size:10px;font-family:'DM Mono',monospace;color:var(--gold);letter-spacing:.12em;text-transform:uppercase;margin-bottom:16px;display:flex;align-items:center;gap:10px}
.card-title::after{content:'';flex:1;height:1px;background:var(--border)}
.shot-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.shot-btn{background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:14px 16px;cursor:pointer;text-align:left;transition:border-color .15s;color:var(--text)}
...
.label{font-size:10px;font-family:'DM Mono',monospace;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}
input[type=text],textarea{width:100%;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:10px 14px;font-family:'DM Mono',monospace;font-size:12px;color:var(--text);outline:none;transition:border-color .15s}
...
.pill{background:var(--surface2);border:1px solid var(--border);border-radius:20px;padding:7px 16px;font-size:12px;font-family:'DM Mono',monospace;color:var(--muted);cursor:pointer;transition:all .15s}
...
.audio-drop{border:1px dashed var(--border2);border-radius:var(--radius);padding:20px;text-align:center;cursor:pointer;position:relative;transition:border-color .15s}
.audio-drop:hover,.audio-drop.has-file{border-color:var(--gold);background:var(--gold-dim)}
.audio-drop.has-file{border-style:solid}
...
.generate-btn{width:100%;max-width:680px;background:linear-gradient(135deg,var(--navy) 0%,var(--navy-light) 100%);color:var(--gold-light);border:1px solid var(--gold);border-radius:var(--radius);padding:16px;font-size:15px;font-weight:600;font-family:'Lora',serif;cursor:pointer;transition:all .2s;margin-bottom:14px}
...
<header>
  <div class="brand">
    <img src="https://raw.githubusercontent.com/benz862/SkillBinder-Video-Creator/main/logo.png" class="brand-logo" alt="SB" onerror="this.style.display='none'"/>
...
<div class="card">
  ...
  <div class="field">
    <div class="label">Upload voiceover or music (MP3 / WAV)</div>
...
<div class="card">
  ...
  <div class="settings-row"><div class="label" style="margin-bottom:8px">Quality</div><div class="pills" id="model-group"><button class="pill active" data-value="kling3_0" onclick="selectPill(this,'model-group')">Turbo — faster</button><button class="pill" data-value="seedance_2_0" onclick="selectPill(this,'model-group')">Standard — best quality</button></div></div>
</div>
<div class="card">
...
<div class="status-card" id="status-card">
  <div class="status-line"><div class="dot" id="status-dot"></div><span id="status-text">Submitting...</span></div>
...
<script>
let selectedShot=document.querySelector('.shot-btn.active').dataset.value,audioMediaId=null,pollTimer=null;
function selectShot(b){document.querySelectorAll('.shot-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');selectedShot=b.dataset.value}
function selectPill(b,g){document.querySelectorAll('#'+g+' .pill').forEach(x=>x.classList.remove('active'));b.classList.add('active')}
function getPill(g){const a=document.querySelector('#'+g+' .pill.active');return a?a.dataset.value:''}
async function fetchBalance(){try{const r=await fetch('/api/balance');const d=await r.json();const c=d.credits??d.balance??d.credit_balance??JSON.stringify(d);document.getElementById('balance-val').textContent=typeof c==='number'?Math.round(c):c}catch(e){document.getElementById('balance-val').textContent='—'}}
async function handleAudioFile(input){const file=input.files[0];if(!file)return;document.getElementById('audio-drop').classList.add('has-file');document.getElementById('audio-file-name').textContent='Uploading...';try{const reader=new FileReader();reader.onload=async(e)=>{const base64=e.target.result.split(',')[1];const res=await fetch('/api/upload-audio',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:file.name,contentType:file.type||'audio/mpeg',fileData:base64})});const data=await res.json();if(!res.ok)throw new Error(data.error||'Upload failed');audioMediaId=data.url||data.media_id||data.id;document.getElementById('audio-file-name').textContent='✓ '+file.name+' — ready'};reader.readAsDataURL(file)}catch(e){document.getElementById('audio-file-name').textContent='Failed: '+e.message}}
async function generate(){
  ...
  const ratio=getPill('ratio-group'),model=getPill('model-group'),extra=document.getElementById('extra').value.trim();
  ...
  let prompt=selectedShot+'. Smooth continuous camera motion, photorealistic, cinematic style.';
  if(extra)prompt+=' '+extra;
  const btn=document.getElementById('gen-btn');
  btn.disabled=true;btn.textContent='Submitting...';
  const sc=document.getElementById('status-card');sc.classList.add('visible');
  document.getElementById('status-dot').className='dot';
  document.getElementById('status-text').textContent='Submitting to Higgsfield...';
  document.getElementById('job-id-label').textContent='';
  document.getElementById('error-msg').style.display='none';
  document.getElementById('result-area').innerHTML='';
  try{
    const r=await fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model,prompt,aspect_ratio:ratio,duration,medias})});
    const data=await r.json();
    if(!r.ok)throw new Error(JSON.stringify(data));
    const jobId=data.request_id||data.id||data.job_id;
    document.getElementById('job-id-label').textContent='Job: '+jobId;
    document.getElementById('status-text').textContent='Generating — 2–3 minutes...';
...
async function pollJob(jobId){
  ...
  try{
    const r=await fetch('/api/job/'+jobId);const data=await r.json();
    const status=(data.status||'').toLowerCase();
    if(status==='completed'||status==='done'||status==='succeeded'){
      const url=data.video?.url||data.url||data.video_url||(data.images&&data.images[0]?.url);
      ...
      if(url)document.getElementById('result-area').innerHTML='<video controls autoplay loop src="'+url+'"></video><a href="'+url+'" download class="dl-btn">Download video</a>';
      updateHistory(jobId,'done');renderHistory();
      document.getElementById('gen-btn').disabled=false;document.getElementById('gen-btn').textContent='Generate Video';
      fetchBalance();
    }else if(status==='failed'||status==='error'||status==='nsfw'||status==='cancelled'){
      document.getElementById('status-dot').className='dot error';
      ...
      document.getElementById('gen-btn').disabled=false;document.getElementById('gen-btn').textContent='Generate Video';
    }else{pollTimer=setTimeout(()=>pollJob(jobId),6000)}
  }catch(e){pollTimer=setTimeout(()=>pollJob(jobId),8000)}
}
function saveHistory(i){const h=JSON.parse(localStorage.getItem('svc_h')||'[]');h.unshift(i);localStorage.setItem('svc_h',JSON.stringify(h.slice(0,20)))}
function updateHistory(id,s){const h=JSON.parse(localStorage.getItem('svc_h')||'[]');const i=h.find(x=>x.jobId===id);if(i)i.status=s;localStorage.setItem('svc_h',JSON.stringify(h))}
function renderHistory(){const h=JSON.parse(localStorage.getItem('svc_h')||'[]');if(!h.length)return;document.getElementById('history-card').style.display='block';document.getElementById('history-list').innerHTML=h.map(i=>'<div class="history-item"><div class="history-meta"><div class="history-shot">'+(i.shot||'Video')+'</div><div class="history-time">'+i.time+' · '+(i.jobId?i.jobId.slice(0,14)+'...':'')+'</div></div><div class="history-status">'+i.status+'</div></div>').join('')}
fetchBalance();renderHistory();
</script>
</body>
</html>`;
}
